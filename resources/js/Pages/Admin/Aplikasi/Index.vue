<template>
    <app-layout title="Aplikasi" menu="aplikasi">
        <template #header>
            <div>
                            ...
                            <transition enter-active-class="transition duration-100 ease-out"
                                ...
                                <MenuItems class="right-0 w-48 origin-top-right component-dropdown">
                                    <div class="py-1">
                                        ...
                                        <button type="button" :class="['component-dropdown-item group w-full']">
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary size="md" as="link" :href="route('admin.aplikasi.create')" class="text-sm">
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Aplikasi
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
            <div class="inline-block min-w-full py-2 align-middle">
                <div
                    ...
                    <table class="min-w-full border-separate sm:table-rounded-lg" style="border-spacing: 0">
                        <thead>
                            <tr>
                                <th scope="col"
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in aplikasi.data">
                                <td :class="[
                                    ...
                                    <div class="text-xs italic text-gray-300" v-if="item.deleted_at">
                                        <UseTimeAgo v-slot="{ timeAgo }" :time="Date.parse(item.deleted_at)">Deleted {{
                                                timeAgo
                                        }}</UseTimeAgo>
                                    </div>
                                </td>
                                ...
                                <td
                                    ...
                                    <ActionTable :edit-link="
                                        route('admin.aplikasi.edit', {
                                            aplikasi: item.id,
                                        })
                                    " :delete-link="!item.deleted_at" @delete="() => (aplikasiBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('aplikasi.restore', {
                                                    aplikasi: item.id,
                                                })
                                                : null
                                        ">
                                        <template #append-menu>
                                            <MenuItem>
                                                ...
                                                >
                                                    <icon-tabler-eye class="w-5 h-5 mr-3 text-brand-secondary/60 group-hover:text-brand-secondary"
                                                    aria-hidden="true" />Open Link
                                                </a>
                                            </MenuItem>
                                        </template>
            ...
            <Pagination size="md" :params="filters" :links="aplikasi?.links" :prev="aplikasi?.prev" :next="aplikasi?.next"
                :only="['aplikasi', 'filters', 'inertiajs']" />
        </div>
        <jet-confirmation-modal :show="aplikasiBeingDeleted !== null" @close="aplikasiBeingDeleted = null">
            <template #title>Delete Aplikasi {{ aplikasiBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this aplikasi?</template>

            <template #footer>
                <ButtonWhite @click="aplikasiBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger class="ml-3" @click="deleteAplikasi" :class="{ 'opacity-25': deleteAplikasiForm.processing }"
                    :disabled="deleteAplikasiForm.processing">Delete</ButtonDanger>
            </template>
...
const openImportNews = ref(false);

const aplikasiBeingDeleted = ref(null);
const deleteAplikasiForm = useForm();

const deleteAplikasi = () =>
    deleteAplikasiForm.delete(
        route('admin.aplikasi.destroy', {
            aplikasi: aplikasiBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (aplikasiBeingDeleted.value = null),
        }
    );
</script>
