<template>
    <app-layout title="News" menu="news">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary size="md" as="link" :href="route('admin.news.create')" class="text-sm">
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create News
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in news.data">
                                ...
                                <td
                                    ...
                                    " :delete-link="!item.deleted_at" @delete="() => (newsBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('news.restore', {
                                                    news: item.id,
            ...
            <Pagination size="md" :params="filters" :links="news?.links" :prev="news?.prev" :next="news?.next"
                :only="['news', 'filters', 'inertiajs']" />
        </div>
        <jet-confirmation-modal :show="newsBeingDeleted !== null" @close="newsBeingDeleted = null">
            <template #title>Delete News {{ newsBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this news?</template>

            <template #footer>
                <ButtonWhite @click="newsBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger class="ml-3" @click="deleteNews" :class="{ 'opacity-25': deleteNewsForm.processing }"
                    :disabled="deleteNewsForm.processing">Delete</ButtonDanger>
            </template>
