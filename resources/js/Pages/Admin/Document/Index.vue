<template>
    <app-layout title="Document" menu="document">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary size="md" as="link" :href="route('admin.document.create')" class="text-sm">
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Document
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
                ...
                <div
                    ...
                    <table class="min-w-full border-separate sm:table-rounded-lg" style="border-spacing: 0">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in document.data">
                                <td :class="[
                                ...
                                <td
                                    ...
                                    " :delete-link="!item.deleted_at" @delete="() => (documentBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
            ...
            <Pagination size="md" :params="filters" :links="document?.links" :prev="document?.prev" :next="document?.next"
                :only="['document', 'filters', 'inertiajs']" />
        </div>
        <jet-confirmation-modal :show="documentBeingDeleted !== null" @close="documentBeingDeleted = null">
            <template #title>Delete Document {{ documentBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this document?</template>

            <template #footer>
                <ButtonWhite @click="documentBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger class="ml-3" @click="deleteDocument" :class="{ 'opacity-25': deleteDocumentForm.processing }"
                    :disabled="deleteDocumentForm.processing">Delete</ButtonDanger>
            </template>
...
const openImportNewsDocument = ref(false);

const documentBeingDeleted = ref(null);
const deleteDocumentForm = useForm();

const deleteDocument = () =>
    deleteDocumentForm.delete(
        route('admin.document.destroy', {
            document: documentBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (documentBeingDeleted.value = null),
        }
    );
</script>
