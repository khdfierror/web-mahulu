<template>
    <app-layout :title="params?.title" menu="wbs-categories">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('admin.wbs.kategori.create')"
                            class="text-sm"
                        >
        ...
        <div class="mt-6">
                ...
                >
                    ...
                    >
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in kategori.data">
                                <td
                                    :class="[
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('admin.wbs.kategori.edit', {
                                                kategori: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (kategoriBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('admin.wbs.kategori.restore', {
                                                      kategori: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="kategori?.prev"
                :next="kategori?.next"
                :only="['kategori', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="kategoriBeingDeleted !== null"
            @close="kategoriBeingDeleted = null"
        >
            <template #title>Delete kategori {{ kategoriBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this kategori?</template>

            <template #footer>
                <ButtonWhite @click="kategoriBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deletekategori"
                    :class="{ 'opacity-25': deletekategoriForm.processing }"
                    :disabled="deletekategoriForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportkategori = ref(false);

const kategoriBeingDeleted = ref(null);
const deletekategoriForm = useForm();

const deletekategori = () =>
    deletekategoriForm.delete(
        route('admin.wbs.kategori.destroy', {
            kategori: kategoriBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (kategoriBeingDeleted.value = null),
        }
    );
</script>
