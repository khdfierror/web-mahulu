<template>
    <app-layout :title="params?.title" menu="gratifikasi.peristiwa">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('admin.gratifikasi.peristiwa.create')"
                            class="text-sm"
                        >
        ...
        <div class="mt-6">
                ...
                >
                    ...
                    >
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in peristiwa.data">
                                <td
                                    :class="[
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('admin.gratifikasi.peristiwa.edit', {
                                                peristiwa: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (peristiwaBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('admin.gratifikasi.peristiwa.restore', {
                                                      peristiwa: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                size="md"
                :params="filters"
                :links="peristiwa?.links"
                :prev="peristiwa?.prev"
                :next="peristiwa?.next"
                :only="['peristiwa', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="peristiwaBeingDeleted !== null"
            @close="peristiwaBeingDeleted = null"
        >
            <template #title>Delete peristiwa {{ peristiwaBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this peristiwa?</template>

            <template #footer>
                <ButtonWhite @click="peristiwaBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deleteperistiwa"
                    :class="{ 'opacity-25': deleteperistiwaForm.processing }"
                    :disabled="deleteperistiwaForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportperistiwa = ref(false);

const peristiwaBeingDeleted = ref(null);
const deleteperistiwaForm = useForm();

const deleteperistiwa = () =>
    deleteperistiwaForm.delete(
        route('admin.gratifikasi.peristiwa.destroy', {
            peristiwa: peristiwaBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (peristiwaBeingDeleted.value = null),
        }
    );
</script>
