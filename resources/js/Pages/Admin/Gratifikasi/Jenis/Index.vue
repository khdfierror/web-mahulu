<template>
    <app-layout :title="params?.title" menu="gratifikasi.jenis">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('admin.gratifikasi.jenis.create')"
                            class="text-sm"
                        >
        ...
        <div class="mt-6">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in jenis.data">
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('admin.gratifikasi.jenis.edit', {
                                                jenis: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (jenisBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('admin.gratifikasi.jenis.restore', {
                                                      jenis: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="jenis?.prev"
                :next="jenis?.next"
                :only="['jenis', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="jenisBeingDeleted !== null"
            @close="jenisBeingDeleted = null"
        >
            <template #title>Delete jenis {{ jenisBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this jenis?</template>

            <template #footer>
                <ButtonWhite @click="jenisBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deletejenis"
                    :class="{ 'opacity-25': deletejenisForm.processing }"
                    :disabled="deletejenisForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportjenis = ref(false);

const jenisBeingDeleted = ref(null);
const deletejenisForm = useForm();

const deletejenis = () =>
    deletejenisForm.delete(
        route('admin.gratifikasi.jenis.destroy', {
            jenis: jenisBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (jenisBeingDeleted.value = null),
        }
    );
</script>
