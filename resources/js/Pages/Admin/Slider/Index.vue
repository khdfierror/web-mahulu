<template>
    <app-layout title="Slider" menu="slider">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('kedeka::admin.slider.create')"
                            class="text-sm"
                        >
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Slider
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
                    ...
                    >
                        <thead>
                            <tr>
                                ...
                                <th
                                    scope="col"
                                    class="sticky top-0 z-10 border-b border-gray-100 dark:border-brand-secondary/20 bg-opacity-75 py-3.5 px-3 text-left text-sm font-normal text-brand-secondary dark:text-slate-400 sm:table-cell"
                                >
                                    Title
                                </th>
                                <th
                            ...
                            <tr v-for="item in slider.data">
                                ...
                                >
                                    <Image
                                        ...
                                        :alt="item.image.name"
                                        preview
                                        class="object-cover h-12 transition-all rounded aspect-[4/3] ring-2 ring-transparent hover:ring-brand-primary/80"
                                    />
                                </td>
                                <td
                                    :class="[
                                        'py-2 px-3 text-sm font-medium border-b border-gray-100 dark:border-brand-secondary/20 whitespace-nowrap',
                                        item.deleted_at
                                            ? 'italic text-gray-400'
                                            : 'text-brand-dark dark:text-white',
                                    ]"
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('kedeka::admin.slider.edit', {
                                                slider: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (sliderBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('slider.restore', {
                                                      slider: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="slider?.prev"
                :next="slider?.next"
                :only="['slider', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="sliderBeingDeleted !== null"
            @close="sliderBeingDeleted = null"
        >
            <template #title>Delete Slider {{ sliderBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this Slider?</template>

            <template #footer>
                <ButtonWhite @click="sliderBeingDeleted = null">Cancel</ButtonWhite>
...
const openImportSlider = ref(false);
...
const deleteSlider = () =>
    deleteSliderForm.delete(
        route('kedeka::admin.slider.destroy', {
            slider: sliderBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
