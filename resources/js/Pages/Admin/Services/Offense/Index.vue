<template>
    <app-layout title="Offense" menu="services.case">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('admin.services.case.create')"
                            class="text-sm"
                        >
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Pelaporan
                            Kasus
                        </ButtonSecondary>
                    </div>
                </div>
        ...
        <div class="mt-6">
                    ...
                    >
                        <thead>
                            <tr>
                                ...
                                >
                                    <div class="flex text-center">Nama Pelapor</div>
                                ...
                                >
                                    ...
                                    <div class="flex text-center">Dilaporkan Pada</div>
                                </th>
                                <th
                                    scope="col"
                                    class="sticky top-0 z-10 border-b border-gray-100 dark:border-brand-secondary/20 bg-opacity-75 py-3.5 pl-4 pr-3 text-left text-sm font-normal text-brand-secondary dark:text-slate-400 sm:pl-6 lg:pl-8"
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in offense.data">
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('admin.services.case.edit', {
                                                offense: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (offenseBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('admin.services.case.restore', {
                                                      offense: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="offense?.prev"
                :next="offense?.next"
                :only="['offense', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="offenseBeingDeleted !== null"
            @close="offenseBeingDeleted = null"
        >
            <template #title>Delete Case {{ offenseBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this case?</template>

            <template #footer>
                <ButtonWhite @click="offenseBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deleteOffense"
                    :class="{ 'opacity-25': deleteOffenseForm.processing }"
                    :disabled="deleteOffenseForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportOffense = ref(false);

const offenseBeingDeleted = ref(null);
const deleteOffenseForm = useForm();

const deleteOffense = () =>
    deleteOffenseForm.delete(
        route('admin.services.case.destroy', {
            offense: offenseBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (offenseBeingDeleted.value = null),
        }
    );
</script>
