<template>
    <app-layout title="Permit" menu="services.permit">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('admin.services.permit.create')"
                            class="text-sm"
                        >
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Permit
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
                    ...
                    >
                        <thead>
                            <tr>
                                ...
                                >
                                    <div class="flex text-center">Nama Pemohon</div>
                                    <div class="flex text-center">No Telp</div>
                                    <div class="flex text-center">Email</div>
                                </th>
                                <th
                                    scope="col"
                                    class="sticky top-0 z-10 border-b border-gray-100 dark:border-brand-secondary/20 bg-opacity-75 py-3.5 pl-4 pr-3 text-left text-sm font-normal text-brand-secondary dark:text-slate-400 sm:pl-6 lg:pl-8"
                                >
                                    ...
                                    <div class="flex text-center">Kategori Izin</div>
                                </th>
                                <th
                                    scope="col"
                                    class="sticky top-0 z-10 border-b border-gray-100 dark:border-brand-secondary/20 bg-opacity-75 py-3.5 pl-4 pr-3 text-left text-sm font-normal text-brand-secondary dark:text-slate-400 sm:pl-6 lg:pl-8"
                                >
                                    ...
                                    <div class="flex text-center">Konsep Kegiatan</div>
                                </th>
                                <th
                                    scope="col"
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in permit.data">
                                ...
                                >
                                    <div class="flex">{{ item.applicant?.name }}</div>
                                ...
                                >
                                    <div class="flex">{{ item.cagar_budaya?.name }}</div>
                                    <div class="text-xs text-gray-400">
                                        {{ item.category?.name }}
                                    </div>
                                </td>
                                ...
                                >
                                    <div class="flex">
                                        {{ item.start_at?.short }} - {{ item.end_at?.short }}
                                    </div>
                                    <div class="text-xs text-gray-400">
                                        {{ item.activity_concept }}
                                    </div>
                                </td>
                                <td
                                    :class="[
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('admin.services.permit.edit', {
                                                permit: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (permitBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('admin.services.permit.restore', {
                                                      permit: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="permit?.prev"
                :next="permit?.next"
                :only="['permit', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="permitBeingDeleted !== null"
            @close="permitBeingDeleted = null"
        >
            <template #title>Delete Permit {{ permitBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this permit?</template>

            <template #footer>
                <ButtonWhite @click="permitBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deletePermit"
                    :class="{ 'opacity-25': deletePermitForm.processing }"
                    :disabled="deletePermitForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportPermit = ref(false);

const permitBeingDeleted = ref(null);
const deletePermitForm = useForm();

const deletePermit = () =>
    deletePermitForm.delete(
        route('admin.services.permit.destroy', {
            permit: permitBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (permitBeingDeleted.value = null),
        }
    );
</script>
