<template>
    <app-layout title="Data Request" menu="services.data-request">
        <template #header>
            <div>
                <div class="flex items-center">
                    <div class="flex-1">
                        <h2 class="text-2xl font-medium text-brand-dark dark:text-white">
                            Permohonan Data Cagar Budaya
                        </h2>
                    </div>
                                    ...
                                    <div class="py-1">
                                        <MenuItem v-slot="{ active }">
                                            ...
                                            >
                                                ...
                                                />Setting Permohonan Data Cagar Budaya
                                            </button>
                                        </MenuItem>
                                    </div>
                                </MenuItems>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('admin.services.data-request.create')"
                            class="text-sm"
                        >
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Permohonan
                            Data Cagar Budaya
                        </ButtonSecondary>
                    </div>
                </div>
        ...
        <div class="mt-6">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in data.data">
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('admin.services.data-request.edit', {
                                                data: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (dataBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('admin.services.data-request.restore', {
                                                      data: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="data?.prev"
                :next="data?.next"
                :only="['data', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal :show="dataBeingDeleted !== null" @close="dataBeingDeleted = null">
            <template #title>Delete Data {{ dataBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this data?</template>

            <template #footer>
                <ButtonWhite @click="dataBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deleteData"
                    :class="{ 'opacity-25': deleteDataForm.processing }"
                    :disabled="deleteDataForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
...
const openImportData = ref(false);

const dataBeingDeleted = ref(null);
const deleteDataForm = useForm();

const deleteData = () =>
    deleteDataForm.delete(
        route('admin.services.data-request.destroy', {
            data: dataBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (dataBeingDeleted.value = null),
        }
    );
</script>
