<template>
    <app-layout title="Cagar Budaya" menu="services.cagar-budaya">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('admin.services.cagar-budaya.create')"
                            class="text-sm"
                        >
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Cagar
                            Budaya
                        </ButtonSecondary>
                    </div>
                </div>
        ...
        <div class="mt-6">
                    ...
                    >
                        <thead>
                            <tr>
                                ...
                                >
                                    <div class="flex text-center">Nama Cagar Budaya</div>
                                </th>
                                <th
                                    scope="col"
                                    class="sticky top-0 z-10 border-b border-gray-100 dark:border-brand-secondary/20 bg-opacity-75 py-3.5 pl-4 pr-3 text-left text-sm font-normal text-brand-secondary dark:text-slate-400 sm:pl-6 lg:pl-8"
                        ...
                        <tbody>
                            <tr v-if="!cagar_budaya.data?.length">
                                <td
                                    colspan="4"
                            ...
                            <tr v-for="item in cagar_budaya.data">
                                <td
                                    :class="[
                                ...
                                >
                                    <div class="flex">{{ item.cp_name }}</div>
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('admin.services.cagar-budaya.edit', {
                                                cagar_budaya: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (cagarBudayaBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('admin.services.cagar-budaya.restore', {
                                                      cagar_budaya: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                size="md"
                :params="filters"
                :links="cagar_budaya?.links"
                :prev="cagar_budaya?.prev"
                :next="cagar_budaya?.next"
                :only="['cagar_budaya', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="cagarBudayaBeingDeleted !== null"
            @close="cagarBudayaBeingDeleted = null"
        >
            <template #title>Delete Cagar Budaya {{ cagarBudayaBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this cagar budaya?</template>

            <template #footer>
                <ButtonWhite @click="cagarBudayaBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deleteCagarBudaya"
                    :class="{ 'opacity-25': deleteCagarBudayaForm.processing }"
                    :disabled="deleteCagarBudayaForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportCagarBudaya = ref(false);

const cagarBudayaBeingDeleted = ref(null);
const deleteCagarBudayaForm = useForm();

const deleteCagarBudaya = () =>
    deleteCagarBudayaForm.delete(
        route('admin.services.cagar-budaya.destroy', {
            cagar_budaya: cagarBudayaBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (cagarBudayaBeingDeleted.value = null),
        }
    );
</script>
