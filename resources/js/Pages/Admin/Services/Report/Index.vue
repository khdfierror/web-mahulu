<template>
    <app-layout title="Laporan Penemuan ODCB" menu="services.report">
        <template #header>
            <div>
                <div class="flex items-center">
                    <div class="flex-1">
                        <h2 class="text-2xl font-medium text-brand-dark dark:text-white">
                            Laporan Penemuan ODCB
                        </h2>
                    </div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('admin.services.report.create')"
                            class="text-sm"
                        >
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Laporan
                            Penemuan ODCB
                        </ButtonSecondary>
                    </div>
                </div>
        ...
        <div class="mt-6">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in report.data">
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('admin.services.report.edit', {
                                                report: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (reportBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('admin.services.report.restore', {
                                                      report: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="report?.prev"
                :next="report?.next"
                :only="['report', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="reportBeingDeleted !== null"
            @close="reportBeingDeleted = null"
        >
            <template #title>Delete Report {{ reportBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this report?</template>

            <template #footer>
                <ButtonWhite @click="reportBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deleteReport"
                    :class="{ 'opacity-25': deleteReportForm.processing }"
                    :disabled="deleteReportForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportReport = ref(false);

const reportBeingDeleted = ref(null);
const deleteReportForm = useForm();

const deleteReport = () =>
    deleteReportForm.delete(
        route('admin.services.report.destroy', {
            report: reportBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (reportBeingDeleted.value = null),
        }
    );
</script>
