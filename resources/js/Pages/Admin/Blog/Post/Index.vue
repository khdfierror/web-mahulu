<template>
    <app-layout title="Post" menu="blog.post">
        <template #header>
            <div>
                <div class="flex items-center">
                    <div class="flex-1">
                        <h2 class="text-2xl font-medium text-brand-dark dark:text-white">Posts</h2>
                    </div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('kedeka::admin.blog.post.create')"
                            class="text-sm"
                        >
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Post
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in post.data">
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('kedeka::admin.blog.post.edit', {
                                                post: item.id,
                                            })
                                        "
            ...
            <Pagination
                ...
                :prev="post?.prev"
                :next="post?.next"
                :only="['post', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal :show="postBeingDeleted !== null" @close="postBeingDeleted = null">
            <template #title>Delete Post {{ postBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this Post?</template>

            <template #footer>
                <ButtonWhite @click="postBeingDeleted = null">Cancel</ButtonWhite>
...
const openImportPost = ref(false);

const postBeingDeleted = ref(null);
const deletePostForm = useForm();

const deletePost = () =>
    deletePostForm.delete(
        route('kedeka::admin.blog.post.destroy', {
            post: postBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (postBeingDeleted.value = null),
        }
    );
</script>
