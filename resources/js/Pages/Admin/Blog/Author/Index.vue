<template>
    <app-layout title="Author" menu="blog.author">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('kedeka::admin.blog.author.create')"
                            class="text-sm"
                        >
                            <icon-tabler-circle-plus class="w-5 h-5 mr-2 text-blue-500" />Add Author
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in author.data">
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('kedeka::admin.blog.author.edit', {
                                                author: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (authorBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('kedeka::admin.blog.author.restore', {
                                                      author: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="author?.prev"
                :next="author?.next"
                :only="['author', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="authorBeingDeleted !== null"
            @close="authorBeingDeleted = null"
        >
            <template #title>Delete Author {{ authorBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this Author?</template>

            <template #footer>
                <ButtonWhite @click="authorBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deleteAuthor"
                    :class="{ 'opacity-25': deleteAuthorForm.processing }"
                    :disabled="deleteAuthorForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportAuthor = ref(false);

const authorBeingDeleted = ref(null);
const deleteAuthorForm = useForm();

const deleteAuthor = () =>
    deleteAuthorForm.delete(
        route('kedeka::admin.blog.author.destroy', {
            author: authorBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (authorBeingDeleted.value = null),
        }
    );
</script>
