<template>
    <app-layout title="Page" menu="page">
        <template #header>
            <div>
                <div class="flex items-center">
                    <div class="flex-1">
                        <h2 class="text-2xl font-medium text-brand-dark dark:text-white">Pages</h2>
                    </div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('kedeka::admin.page.create')"
                            class="text-sm"
                        >
                            <icon-tabler-edit class="w-5 h-5 mr-2 text-blue-500" />Create Page
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in page.data">
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('kedeka::admin.page.edit', {
                                                page: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (pageBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('kedeka::admin.page.restore', {
                                                      page: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="page?.prev"
                :next="page?.next"
                :only="['page', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal :show="pageBeingDeleted !== null" @close="pageBeingDeleted = null">
            <template #title>Delete Page {{ pageBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this Page?</template>

            <template #footer>
                <ButtonWhite @click="pageBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deletePage"
                    :class="{ 'opacity-25': deletePageForm.processing }"
                    :disabled="deletePageForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
...
const openImportPage = ref(false);

const pageBeingDeleted = ref(null);
const deletePageForm = useForm();

const deletePage = () =>
    deletePageForm.delete(
        route('kedeka::admin.page.destroy', {
            page: pageBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (pageBeingDeleted.value = null),
        }
    );
</script>
