<template>
    <app-layout title="Question" menu="survey.question">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('kedeka::admin.survey.question.create')"
                            class="text-sm"
                        >
        ...
        <div class="mt-6">
                ...
                >
                    ...
                    >
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in question.data">
                                <td
                                    :class="[
                                ...
                                >
                                    ...
                                    <div
                                        class="mt-2 text-xs text-brand-secondary dark:text-slate-500"
                                    >
                                        <ul class="max-w-sm list-disc list-inside">
                                            <li
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('kedeka::admin.survey.question.edit', {
                                                question: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (questionBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('kedeka::admin.survey.question.restore', {
                                                      question: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="question?.prev"
                :next="question?.next"
                :only="['question', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="questionBeingDeleted !== null"
            @close="questionBeingDeleted = null"
        >
            <template #title>Delete Question {{ questionBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this Question?</template>

            <template #footer>
                <ButtonWhite @click="questionBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deleteQuestion"
                    :class="{ 'opacity-25': deleteQuestionForm.processing }"
                    :disabled="deleteQuestionForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportQuestion = ref(false);

const questionBeingDeleted = ref(null);
const deleteQuestionForm = useForm();

const deleteQuestion = () =>
    deleteQuestionForm.delete(
        route('kedeka::admin.survey.question.destroy', {
            question: questionBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (questionBeingDeleted.value = null),
        }
    );
</script>
