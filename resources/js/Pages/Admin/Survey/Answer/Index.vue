<template>
    <app-layout title="Answer" menu="survey.answer">
        <template #header>
            <div>
                ...
                <div class="flex items-center mt-3">
                    ...
                    <div class="inline-flex items-center ml-auto">
                        <ButtonSecondary
                            size="md"
                            as="link"
                            :href="route('kedeka::admin.survey.answer.create')"
                            class="text-sm"
                        >
                            <icon-tabler-circle-plus class="w-5 h-5 mr-2 text-blue-500" />Add Answer
                        </ButtonSecondary>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
                ...
                >
                    ...
                    >
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in answer.data">
                                <td
                                    :class="[
                                        'py-2 pl-4 pr-3 text-sm font-medium border-b border-gray-100 dark:border-brand-secondary/20 sm:pl-6 lg:pl-8',
                                        item.deleted_at
                                            ? 'italic text-gray-400'
                                            : 'text-brand-dark dark:text-white',
                                    ]"
                                >
                                    <div class="flex max-w-sm">{{ item.question }}</div>
                                    <div
                                ...
                                <td
                                    :class="[
                                        'hidden px-3 py-2 text-sm border-b border-gray-100 dark:border-brand-secondary/20 sm:table-cell',
                                        item.deleted_at
                                            ? 'italic text-gray-400'
                                            : 'text-brand-secondary dark:text-brand-secondary',
                                    ]"
                                ...
                                >
                                    <ActionTable
                                        :edit-link="
                                            route('kedeka::admin.survey.answer.edit', {
                                                answer: item.id,
                                            })
                                        "
                                        :delete-link="!item.deleted_at"
                                        @delete="() => (answerBeingDeleted = item)"
                                        :restore-link="
                                            item.deleted_at
                                                ? route('kedeka::admin.survey.answer.restore', {
                                                      answer: item.id,
                                                  })
                                                : null
                                        "
            ...
            <Pagination
                ...
                :prev="answer?.prev"
                :next="answer?.next"
                :only="['answer', 'filters', 'inertiajs']"
            />
        </div>
        <jet-confirmation-modal
            :show="answerBeingDeleted !== null"
            @close="answerBeingDeleted = null"
        >
            <template #title>Delete Answer {{ answerBeingDeleted?.title }}</template>

            <template #content>Are you sure you would like to delete this Answer?</template>

            <template #footer>
                <ButtonWhite @click="answerBeingDeleted = null">Cancel</ButtonWhite>

                <ButtonDanger
                    class="ml-3"
                    @click="deleteAnswer"
                    :class="{ 'opacity-25': deleteAnswerForm.processing }"
                    :disabled="deleteAnswerForm.processing"
                    >Delete</ButtonDanger
                >
            </template>
        </jet-confirmation-modal>
...
const openImportAnswer = ref(false);

const answerBeingDeleted = ref(null);
const deleteAnswerForm = useForm();

const deleteAnswer = () =>
    deleteAnswerForm.delete(
        route('kedeka::admin.survey.answer.destroy', {
            answer: answerBeingDeleted.value?.id,
        }),
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => (answerBeingDeleted.value = null),
        }
    );
</script>
